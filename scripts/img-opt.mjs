...
//   1. 读取源目录 assets-src/viz/ 下的 SVG 图片（团队产出位置）
//   2. 用 svgo 做矢量压缩（去掉注释/空白/冗余属性/元数据）
//   3. 用 sharp 为每个 SVG 生成 PNG 栅格 fallback（供不支持 SVG 的环境）
//   4. 将优化产物写入 public/assets/viz/ —— 即 VitePress 的 public 目录
//      （config.mts 中 publicDir: 'public'，相对项目根）。
//      注意：.vitepress/public/ 不是 VitePress 的 public 目录，写那里不会进 dist/
//      （曾踩坑：img-opt 产物从未被构建使用，站点一直用旧副本）。
...
//   注意：根目录下不得存在 assets/viz/ 同名文件（源 SVG 在 assets-src/viz/）。
//   否则 Vite 会把 /assets/viz/*.svg 解析为【源码资源】打包加 hash，
//   而不会命中 public 的优化产物（曾踩坑：页面一直加载未压缩源 SVG，
//   img-opt 的压缩/PNG/深色适配全部未生效）。
...
// 源目录：团队在仓库根 assets-src/viz/ 产出 SVG
// （注意：不得放在根 assets/viz/，否则会与 public URL /assets/viz/ 冲突，见上方引用规范注释）
const SRC = resolve(root, 'assets-src/viz')
// 产物目录：VitePress public 目录（config.mts 的 publicDir: 'public'，构建时原样拷贝进 dist/，base 前缀自动加）
const OUT = resolve(root, 'public/assets/viz')
